...
def load_data() -> list[NorseDoc]:
    if not os.path.exists(MENOTA_PICKLE):
        parsed_docs_list = import_menota_data(path= OLD_NORSE_CORPUS_FILES, test=False)
...
def text_to_sequence(texts: list[tuple[list[str], str]], word_index: dict[str, int]) -> list[list[int]]:
    sequences = []
    for text in texts:
