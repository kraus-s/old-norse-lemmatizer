...
def vectorize_doc_list(docs_list: list[list[str]], min_occurence: int = 5):
    ...
    vectorized_model = Word2Vec(docs_list_windowed, min_count=min_occurence, window=3, workers=8, vector_size=VECTOR_SPACE)
    return vectorized_model


def load_data() -> list[NorseDoc]:
    if not os.path.exists(MENOTA_PICKLE):
        parsed_docs_list = importer.import_menota_data(path= OLD_NORSE_CORPUS_FILES, test=False)
        pickle.dump(parsed_docs_list, open(MENOTA_PICKLE, "wb"))
    else:
        parsed_docs_list = pickle.load(open(MENOTA_PICKLE, "rb"))
...
def _preprocess_data_for_prediction(docs_list: list[NorseDoc], word_index: dict[str, int], sampling: bool = False, stand_off: bool = False, ambiguous_forms: dict[str, set[str]] = None) -> tuple[np.ndarray, list[tuple[list[str], str]]]:
    ...
    non_lemmatized_docs = [x for x in docs_list if not x.lemmatized]
    tokenized_list = [tok for doc in non_lemmatized_docs for tok in doc.tokens if tok.normalized in word_index.keys()]
    if stand_off:
    ...
    if sampling:
        sequences = random.sample(sequences, k=20)
    sequences_seq = importer.text_to_sequence([x[0] for x in sequences], word_index)
...
if __name__ == "__main__":
    ...
    # stop_filtered_docs_list = _lemma_based_stop_filter(best_list)
    ...
    model.fit(x_train_pad, y_train_cat, epochs=15, validation_split=0.2, use_multiprocessing=True, workers=8, batch_size=32)
